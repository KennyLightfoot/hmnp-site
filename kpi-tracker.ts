...
export class KPITracker {
  ...
  private async updateDailyMetrics(booking: BookingKPI): Promise<void> {
    ...
    if (!existing.serviceBreakdown[serviceType]) {
      existing.serviceBreakdown[serviceType] = { count: 0, revenue: 0 };
    }
    existing.serviceBreakdown[serviceType].count++;
    if (booking.status === 'completed') {
      existing.serviceBreakdown[serviceType].revenue += booking.totalValue;
    }

    // Update location breakdown
    const city = booking.location.city;
    const cityKey = city || "unknown";
    if (!existing.locationBreakdown[cityKey]) {
      existing.locationBreakdown[cityKey] = { count: 0, averageDistance: 0, totalTravelFees: 0 };
    }
    existing.locationBreakdown[cityKey].count++;
    existing.locationBreakdown[cityKey].totalTravelFees += booking.travelFee;
  ...
  private calculateServiceBreakdown(bookings: BookingKPI[]): { [key: string]: { count: number; revenue: number } } {
    return bookings.reduce((breakdown, booking) => {
      ...
      if (!breakdown[serviceType]) {
        breakdown[serviceType] = { count: 0, revenue: 0 };
      }
      breakdown[serviceType].count++;
      breakdown[serviceType].revenue += booking.totalValue;
      return breakdown;
  ...
  private calculateLocationBreakdown(bookings: BookingKPI[]): { [key: string]: { count: number; averageDistance: number; totalTravelFees: number } } {
    ...
    bookings.forEach(booking => {
      const city = booking.location.city;
      const cityKey = city || "unknown";
      if (!breakdown[cityKey]) {
        breakdown[cityKey] = { count: 0, totalDistance: 0, totalTravelFees: 0 };
      }
      breakdown[cityKey].count++;
      breakdown[cityKey].totalDistance += booking.distance;
      breakdown[cityKey].totalTravelFees += booking.travelFee;
    });
