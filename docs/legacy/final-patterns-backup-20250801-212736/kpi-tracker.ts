...
export class KPITracker {
  ...
  private async updateDailyMetrics(booking: BookingKPI): Promise<void> {
    ...
    if (booking.status === 'completed') {
      existing.total_revenue += booking.totalValue;
    ...
    if (!existing.locationBreakdown[city || "unknown"]) {
      existing.locationBreakdown[city || "unknown"] = { count: 0, averageDistance: 0, totalTravelFees: 0 };
    }
    existing.locationBreakdown[city || "unknown"].count++;
    existing.locationBreakdown[city || "unknown"].totalTravelFees += booking.travelFee;

    // Update other metrics
    existing.averageBookingValue = existing.total_revenue / Math.max(existing.total_bookings, 1);
  ...
  private calculateLocationBreakdown(bookings: BookingKPI[]): { [key: string]: { count: number; averageDistance: number; totalTravelFees: number } } {
    ...
    bookings.forEach(booking => {
      const city = booking.location.city;
      if (!breakdown[city || "unknown"]) {
        breakdown[city || "unknown"] = { count: 0, totalDistance: 0, totalTravelFees: 0 };
      }
      breakdown[city || "unknown"].count++;
      breakdown[city || "unknown"].totalDistance += booking.distance;
      breakdown[city || "unknown"].totalTravelFees += booking.travelFee;
    });
