# HMNP Mobile Notary Services - Fee Schedule

## ⚖️ Texas-Compliant Notary Fees
**Statutory Notary Fees (Included in Total Price)**
| Service | TX Max | Our Fee |
|---------|--------|---------|
| Acknowledgments | $10 | $6 |
| Jurats | $10 | $8 |
| Oaths | $10 | $8 |

### Basic Notarial Acts
| Service | Our Fee | State Maximum |
|---------|---------|---------------|
| First signature | $6 | $10 |
| Additional signatures | $1 | $1 |
| Oaths/affirmations | $8 | $10 |
| Depositions | $1/100 words | $1/100 words |
| Certificates under seal | $10 | $10 |

### Service Fees (Non-Notarial)
*Separate from statutory notarial fees per Texas law*
- Travel time
- Document preparation
- Mobile service
- Administrative costs
- Technology fees

## 📱 Core Mobile Notary Services

### 📦 Essential Mobile Package
- **Base Service**: $75 (Meets TX minimum)
  - 1 signer
  - 2 documents
  - 15-mile radius
- **Add-ons**:
  - ➕ Additional signer: $25
  - ➕ Extra document: $15
  - ➕ Weekend service: +$50

- **2 Signers**: $85
  - Includes up to 3 documents notarized per signer

- **3 Signers**: $95
  - Includes up to 3 documents notarized per signer

- **4+ Signers**: Custom Quote
  - Contact us for a tailored package

**Additional Details**
- **Extra Documents**: $15 each (beyond 2)
- **Extended Travel**: $0.50/mile beyond 20-mile radius
- **Service Hours**: Monday-Friday, 9am-5pm

### 🚨 Priority Service Package
**Base Rate: $100**
- Documents: Up to 5
- Signers: 2
- Service Area: 35-mile extended radius (+$0.50/mile beyond 20 miles)
- Service Hours: 7am-9pm daily

**Includes:**
- ✅ 2-hour response time
- ✅ Priority Service Hours (7am-9pm)
- ✅ Certified mail preparation
- ✅ SMS status tracking

**Additional Fees:**
- ➕ Extra signers: $10/each

## 🏡 Loan Signing Specialties

### Standard Loan Closing
**Base Rate: $150**

**Package Includes:**
- ✅ Unlimited documents
- ✅ Up to 4 signers
- ✅ Color printing service
- ✅ Professional closing binder
- ✅ Title company shipping
- ✅ 90-minute signing session
- ➕ Overnight document handling: +$35

### Reverse Mortgage/HELOC
**Base Rate: $150**

**Package Includes:**
- ✅ 4-hour response window
- ✅ Certified mail return
- ✅ Dual agent coordination

## 🌟 Specialized Services

### 🛂 Apostille Services
- **Apostille Services**: $75 + state fees
- Expedited Processing: +$75
- Document Preparation: $20/document

### 🔐 Verification Services
- Background Check Verification: $55
- Wedding Certificate Expediting: $75
- **Medallion Signature**: $150 (High-Risk Handling Fee)

## 📦 Additional Services

### Document Services
- Printing:
  - Black & White: $0.30/page
  - Color: $1.00/page
- Secure Cloud Storage: $15/month
- Basic Fax/Scan: $15
- Secure Email Delivery: $10
- USPS Certified Mail: $20

### Premium Time Slots
- 📅 Sunday & Federal Holidays: +$40
- 🌎 Bilingual Services (Spanish/English): +$20

## 🚨 Policies

### Cancellation Policy
- Less than 2 hours notice: $35
- Pre-approved weather emergencies: 15% reschedule discount
- No-show fee: $50 + travel costs
- Signing refusal: $75 documentation fee

### Travel Policy
- Free within 20-mile radius
- Beyond 20 miles: $0.50/mile
- Toll fees: Reimbursed at cost

## 💡 Client Benefits

### New Client Offers
- First-time customers: 10% off services under $200
- Referral program: $25 credit per successful referral

### Value-Added Services
- Digital document archiving: Free for 1 year
- Real-time SMS updates during service hours: Included
- Mobile tracking: Included

## 📋 Fee Disclosure Requirements
In compliance with Texas Government Code:
1. This fee schedule is posted at our office location
2. Detailed fee book maintained for all transactions
3. Itemized receipts provided for all services
4. Clear separation of notarial and non-notarial fees

## 🔍 Fee Transparency
- All notarial fees are charged per state maximums
- Service fees are separate from notarial acts
- Itemized invoices provided for all services
- Corporate accounts billed with detailed fee breakdown

*Note: This fee schedule complies with Texas Government Code §406.024. Service fees are separate from statutory notarial fees and represent additional services provided beyond basic notarization.*

---

**Reference:** [Texas Secretary of State Notary Fee Guidelines](https://notarytraining.sos.texas.gov/notaryfees)

## 🤝 Professional Service Packages

### 💼 Title Company Partnership
**Base Rate: $125/month**
- Priority scheduling for closings
- Dedicated phone line
- Volume pricing:
  - 1-10 signings: $140/each
  - 11-20 signings: $130/each
  - 21+ signings: $120/each
- Monthly reporting and analytics

### 🏢 Business Concierge Package
**Base Rate: $200/month**
- Up to 10 standard notarizations
- On-site service twice monthly
- Scheduled office hours
- Employee document verification
- Corporate billing options
- Dedicated account manager

### 🏥 Healthcare Provider Package
**Base Rate: $175/month**
- HIPAA-compliant document handling
- Medical facility priority service
- Patient room visits
- Healthcare POA expertise
- Staff training sessions
- Emergency response (within 90 minutes)

### ⚖️ Estate Planning Package
**Base Rate: $250**

**A comprehensive package designed for individuals and families preparing their essential estate documents.**

**Package Includes:**
- ✅ **Up to 10 Documents:** Notarization for any combination of Wills, POAs, Trusts, or other estate-related documents.
- ✅ **Up to 4 Signers:** Perfect for couples or individuals with multiple designated agents.
- ✅ **90-Minute Appointment:** A dedicated, unhurried session to ensure all documents are handled correctly.
- ✅ **Mobile Service:** Includes travel within our standard 20-mile radius.
- ✅ **Itemized Receipt:** A clear, detailed receipt for your records or legal counsel.

**Common Add-ons:**
- ➕ **Additional Documents:** $10 each (discounted from $15)
- ➕ **Witness Sourcing:** $50 per witness provided by us.
- ➕ **Extended Travel:** Standard mileage rates apply beyond 20 miles.
- ➕ **Weekend & Holiday Service:** +$40

### 📚 Education Institution Package
**Base Rate: $150/month**
- Student ID verification services
- International student document processing
- Faculty/Staff priority service
- Campus office hours
- Bulk processing discounts
- Academic document certification

### 🏗️ Construction & Real Estate Developer Package
**Base Rate: $250/month**
- Multiple site visits
- Contractor document processing
- Lien release handling
- Permit documentation
- Progress payment notarization
- Project-based billing
- Emergency weekend service

**Service Hours**: Follow Priority Service Hours (7am-9pm daily)

### Payment Security Deposit
| Service Type        | Hold Amount | Conditions               |
|----------------------|-------------|--------------------------|
| Essential Weekday    | $50         | All appointments         |
| Priority Service     | $75         | After 5pm or >15 miles   |
| Weekend/Holiday      | $100        | Applies to all bookings  |

**Texas Notary Fee Compliance**  
| Notarial Act | Your Cost | Our Fee | Total | State Max |
|--------------|-----------|---------|-------|-----------|
| First Signature | $6 | $69 | $75 | $10 |
| Oath (Loan Closing) | $8 | $142 | $150 | $10 |

### 🏡 Loan Signing Services
**$150 Fee Breakdown**:
- Notarial Fees: $14 (2 signatures @ $6+$1×8)
- Service Fees: $136 (Document prep, travel, etc)