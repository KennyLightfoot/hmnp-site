...
export default function BookingSuccessClient() {
  ...
  useEffect(() => {
    ...
    if (bookingId) {
      ...
      if (docsParam) {
        try {
          setUploadedDocs(JSON.parse(decodeURIComponent(docsParam)));
        } catch {
          // no-op
        }
      }
      ...
      try {
        const value =
          typeof booking.totalPrice === 'number'
            ? booking.totalPrice
            : getEstimatedValue(booking.serviceType);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-2xl mx-auto px-4">
          ...
          <Card className="mb-6">
            ...
            <CardContent className="space-y-2 pt-2">
              <p className="text-sm text-muted-foreground">
                Thanks! Your documents were received and attached to this booking.
              </p>
        ...
        <div className="mt-6 grid sm:grid-cols-2 gap-4">
          <Button
            ...
            onClick={async () => {
              try {
                const to =
                  prompt('Enter your mobile number to receive your booking link by SMS:') || '';
                if (!to) return;
                ...
                await fetch('/api/sms/send', {
                  method: 'POST',
                  headers: hdrs,
                  body: JSON.stringify({ to, body: `Your HMNP booking link: ${link}` }),
                });
                try {
                  (window as any).dataLayer = (window as any).dataLayer || [];
              ...
              try {
                ...
                if (start && end) {
                  const href = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(
                    title,
                  )}&dates=${toIso(start)}/${toIso(end)}&details=${encodeURIComponent(details)}`;
        ...
        <div className="text-center mt-8">
          ...
          <p className="text-sm font-medium text-blue-600">Call us at (832) 617-4285</p>
        </div>
      </div>
    </div>
  );
