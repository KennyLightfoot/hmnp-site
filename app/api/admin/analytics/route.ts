    ...
    const analyticsData = {
      ...
      serviceBreakdown: [
        { 
          ...
          bookings: Math.floor(metrics.totalBookings * 0.456), 
          revenue: Math.floor(metrics.totalRevenue * 0.407),
          percentage: 45.6 
        },
        { 
          serviceType: 'Extended Hours', 
          bookings: Math.floor(metrics.totalBookings * 0.287), 
          revenue: Math.floor(metrics.totalRevenue * 0.341),
          percentage: 28.7 
        },
        { 
          serviceType: 'Loan Signing', 
          bookings: Math.floor(metrics.totalBookings * 0.257), 
      ...
      timeAnalytics: metrics.peakDays.map(day => ({
        ...
        revenue: Math.floor((day.bookings / metrics.totalBookings) * metrics.totalRevenue)
      })),
      pricingMetrics: {
      ...
      metadata: {
        ...
        dataPoints: metrics.totalBookings
      }
    };
