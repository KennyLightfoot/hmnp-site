...
echo -e "${BLUE}🔧 Batch 6 Error Fix Script${NC}"
echo "================================"

# Create backup directory
BACKUP_DIR="./batch-6-backup-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
echo -e "${GREEN}Phase 5: Fixing assignment expression issues${NC}"

# Fix "The left-hand side of an assignment expression may not be an optional property access" issues
find . -name "*.ts" -o -name "*.tsx" | grep -v node_modules | grep -v .git | grep -v backup | while read file; do
    if grep -q "The left-hand side of an assignment expression may not be an optional property access" "$file"; then
        echo "  🔧 Fixing assignment expression issues in: $file"
        backup_file "$file"
        
        # Replace optional chaining with null checks for assignment operations
...
echo -e "${GREEN}Phase 6: Fixing argument count issues${NC}"
...
echo -e "${GREEN}Phase 9: Fixing total_revenue property issues${NC}"

# Fix total_revenue property issues
find . -name "*.ts" -o -name "*.tsx" | grep -v node_modules | grep -v .git | grep -v backup | while read file; do
    if grep -q "Property 'total_revenue' does not exist on type 'BusinessMetrics'" "$file"; then
        echo "  🔧 Fixing total_revenue property in: $file"
        backup_file "$file"
        
        # Fix total_revenue property name
...
echo -e "${GREEN}✅ Batch 6 fixes completed!${NC}"
