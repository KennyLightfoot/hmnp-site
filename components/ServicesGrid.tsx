          ...
          {[
            ...
            <div key={s.title} className="rounded-2xl border border-black/5 p-6">
              ...
              <div className="flex items-center gap-3 mt-4">
                <Link href={s.href} className="inline-flex" onClick={() => track('cta_clicked', { cta_name: s.title, location: 'services_grid' })}>
                  <Button className="bg-secondary text-white hover:bg-secondary/90">Book</Button>
                </Link>
                {'learnMore' in s && s.learnMore && (
                  <Link href={s.learnMore} className="text-sm text-secondary/60 underline underline-offset-2 hover:text-secondary transition-colors">
                    Learn more
                  </Link>
                )}
              </div>
